...
async def post_init(application: Application) -> None:
    """Runs once PTB's own event loop is up, before polling starts.
    This is where background tasks belong -- run_polling() manages the
    loop itself, so we can't create tasks against it beforehand."""
    application.create_task(notification_loop())
...
def main() -> None:
    ...
    # run_polling() owns and manages its own event loop -- call it
    # synchronously, not from inside another asyncio.run() loop.
